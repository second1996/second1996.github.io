$(document).ready(function () {
  ...
  function qtyChangeValue(qtyInput, count) {
    ...
    form
      .find('.itemPrice')
      .text((qty.price * qtyVal).toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1 '));
  }
  ...
  window.initShopFilters = function () {
    $('.shop-filters-item').each(function (index, el) {
      ...
      $(el)
        .find('.shop-filters-list input[type="checkbox"]')
        .each(function (index, checkboxEl) {
          if ($(this).is(':checked')) {
  ...
  window.showAllCollectionTabs = function () {
    $('.collection-products-list > [class*="col"]')
      .clone()
      .appendTo('#products-tab-1 .collection-products-list');
  };
